...
ROSScenarioParser::ROSScenarioParser(ROSSimulationManager* sm) : ScenarioParser(sm)
{
    ...
    pubs["simulation_time"] = nh.advertise<std_msgs::Float64>(std::string("/stonefish/simulation_time"), 10);
}
...
bool ROSScenarioParser::ParseRobot(XMLElement* element)
{
    ...
    element->QueryStringAttribute("name", &name1);
    std::string nameTest(name1);
    
        // Get publisher for hydrodynamics
        
    pubs[nameTest + "/hydrodynamics"] =     nh.advertise<stonefish_ros::HydroDynamics>(std::string(nameTest+"/hydrodynamics"), 10);

    //ROS_WARN_STREAM("===="+nameTest);
    //Check actuators
    ...
    if((item = element->FirstChildElement("ros_subscriber")) != nullptr)
    {
        ...
        const char* topicForces = nullptr;
        ...
        if(nThrusters > 0 && item->QueryStringAttribute("forces", &topicForces) == XML_SUCCESS)
            subs[robot->getName() + "/thrusters_forces"] = nh.subscribe<geometry_msgs::WrenchStamped>(std::string(topicForces), 10, ThrustersForcesCallback(sim, rosRobot));
    ...
    if((item = element->FirstChildElement("ros_publisher")) != nullptr)
    {
        ...
        if(nThrusters > 0 && item->QueryStringAttribute("forces", &topicForces) == XML_SUCCESS)
            pubs[robot->getName() + "/thrusters_forces"] = nh.advertise<geometry_msgs::WrenchStamped>(std::string(topicForces), 10);
    ...
    //Compute Thrust Allocation Matrix Pseudo Inverse
    cv::Mat A = cv::Mat(6, nThrusters, CV_64F);
    std::ifstream inputFile(std::string(std::filesystem::current_path())+"/"+robot->getName()+".txt");
    if (!inputFile.is_open()) {
        ROS_ERROR("Error opening thrust allocation matrix of robot %s file.", robot->getName().c_str());
        return 1;
    }

    // Read the matrix values from the file, skipping every other line
    ...
    while (getline(inputFile, line)) {
        ...
        // Skip the next line containing the thruster name
        getline(inputFile, line);
    }
    ...
    // Convert the matrix values to a cv::Mat
    int numRows = matrixValues[0].size();
    int numCols = matrixValues.size();
    for (int i = 0; i < numRows; ++i) {
        for (int j = 0; j < numCols; ++j) {
            A.at<double>(i, j) = matrixValues[j][i];
        }
    }

    // Compute the pseudoinverse using SVD
    cv::Mat A_pseudo;
    cv::invert(A, A_pseudo, cv::DECOMP_SVD);

    // Write the pseudoinverse matrix to a file
    std::ofstream outputFile(robot->getName()+"_pseudo_inverse.txt");
    if (outputFile.is_open()) {
        outputFile << A_pseudo;
        outputFile.close();
        ROS_INFO("Pseudoinverse matrix written to 'pseudo_inverse.txt'.");
    } else {
        ROS_ERROR("Error writing pseudoinverse matrix of robot %s to file.", robot->getName().c_str());
        return false;
    }
    rosRobot->setPseudoInverse(A_pseudo);
...
bool ROSScenarioParser::ParseTether(XMLElement* element)
{
    ROS_INFO("TETHER CHECKING");
    if(!ScenarioParser::ParseTether(element))
        return false;
    return true;
}
            ...
            case SensorType::VISION:
            {
                switch(((VisionSensor*)sens)->getVisionSensorType())
                {
                    ...
                    case VisionSensorType::EVENT_BASED_CAMERA:
                    {
                        ...
                        pubs[sensorName + "/info"] = nh.advertise<sensor_msgs::CameraInfo>(topicStr + "/camera_info", queueSize);
                        ColorCamera* cam = (ColorCamera*)sens;
                        cam->InstallNewDataHandler(std::bind(&ROSSimulationManager::EventCameraImageReady, sim, std::placeholders::_1));
                        camMsgProto[sensorName] = ROSInterface::GenerateCameraMsgPrototypes(cam, false);
                     }
                        break;
