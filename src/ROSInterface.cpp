...
void ROSInterface::PublishIMU(ros::Publisher& pub, IMU* imu, ros::Publisher& pub_heading)
{
    ...
    double heading=atan2(2.0 * (quat.z() * quat.w() + quat.x() * quat.y()) , - 1.0 + 2.0 * (quat.w() * quat.w() + quat.x() * quat.x()))*180/3.14159265;
    stonefish_ros::Compass heading_msg;
...
void ROSInterface::PublishLiDAR360(ros::Publisher& pub, LiDAR360* mb)
{
    Sample sample = mb->getLastSample();
    SensorChannel channel = mb->getSensorChannelDescription(0);
    std::vector<Scalar> distances = sample.getData();
...
std::pair<sensor_msgs::ImagePtr, sensor_msgs::CameraInfoPtr> ROSInterface::GenerateCameraMsgPrototypes(Camera* cam, bool depth)
{
    ...
    info->P[3] = -(cam->getBaseline()*info->K[2]/tanhfov2); //au*Tx - position of second camera from stereo pair
    info->P[7] = 0.0; //Ty;
    info->P[10] = 1.0;
    //ROI
    info->roi.x_offset = 0;
