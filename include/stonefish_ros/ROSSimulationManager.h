...
namespace sf
{
	...
	struct ROSRobot
	{
		...
		void setPseudoInverse(const cv::Mat& pseudo) {
                     TAM_pseudo = pseudo;
               }
               cv::Mat getPseudoInverse() const {
                   return TAM_pseudo;
               }
	};
	...
	public:
	    ...
	    virtual void EventCameraImageReady(ColorCamera* cam);
		...
		double interp(double, const std::vector<double>&, const std::vector<double>&);
		std::vector<double> getRPMs(); 
		std::vector<double> getForces(); 
		std::vector<double> getRPMsConfigVal(); 
		std::vector<double> getForcesConfigVal(); 
		void readForce2RPM();
		 // Vectors to store RPM and force values from conf file
	...
	class ThrustersForcesCallback
	{
	public:
		ThrustersForcesCallback(ROSSimulationManager* sm, ROSRobot* robot);
		void operator()(const geometry_msgs::WrenchStamped::ConstPtr& msg);
